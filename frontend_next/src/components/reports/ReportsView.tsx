...
// Port fidèle de frontend/src/components/reports/ReportsView.jsx (export
// TXT/PDF côté client, jsPDF) — correction du bug d'origine (export PDF
// superadmin manquant, cf. bugs corrigés en amont sur l'app Flask) : ici
// canProjectReport inclut bien isSuperadmin dès le départ.
// p.chef_id (legacy) -> p.user_role === "owner" (le rôle du projet pour
// l'utilisateur courant, calculé côté serializer).
import { useState } from "react";
import { FileText, Download, Users, User, FileDown, FolderOpen } from "lucide-react";
import { jsPDF } from "jspdf";
import * as api from "@/lib/api";
import type { Projet, RapportData, RapportProjet, Utilisateur } from "@/lib/types";
...
function generateTXT(data: RapportData, memberName: string | null) {
  const lines: string[] = [];
  ...
  data.members.forEach((m) => {
    ...
    if (m.done_tasks.length > 0) {
      ...
      m.done_tasks.forEach((t) => lines.push(`    • [${t.id}] ${t.description} (${t.project_name || "Sans projet"}), terminée le ${formatDate(t.completed_at)}`));
    }
    if (m.in_progress_tasks.length > 0) {
      lines.push(`\n  🔄 EN COURS (${m.in_progress_tasks.length})`);
      m.in_progress_tasks.forEach((t) => lines.push(`    • [${t.id}] ${t.description} (${t.project_name || "Sans projet"})`));
    }
    ...
    if (m.difficulties.length > 0) {
      ...
      m.difficulties.forEach((d) => {
        lines.push(`    Tâche [${d.task_id}] ${d.task_description} (${d.project_name || "Sans projet"})`);
...
function generatePDF(data: RapportData, memberName: string | null) {
  ...
  const line = (text: string, size = 10, style: "normal" | "bold" = "normal", color: [number, number, number] = [30, 30, 30]) => {
    ...
    doc.splitTextToSize(text, pageW - margin * 2).forEach((w: string) => {
  ...
  data.members.forEach((m) => {
    ...
    if (m.done_tasks.length > 0) {
      ...
      m.done_tasks.forEach((t) => line(`- [${t.id}] ${t.description} (${t.project_name || "Sans projet"}), terminée le ${formatDate(t.completed_at)}`, 9));
    }
    if (m.in_progress_tasks.length > 0) {
      line(`En cours (${m.in_progress_tasks.length})`, 11, "bold", [40, 90, 180]);
      m.in_progress_tasks.forEach((t) => line(`- [${t.id}] ${t.description} (${t.project_name || "Sans projet"})`, 9));
    }
    if (m.overdue_tasks.length > 0) {
    ...
    if (m.difficulties.length > 0) {
      ...
      m.difficulties.forEach((d) => {
        line(`Tâche [${d.task_id}] ${d.task_description} (${d.project_name || "Sans projet"})`, 9, "bold");
...
function generateProjectTXT(data: RapportProjet) {
  ...
  if (data.done_tasks.length > 0) {
    ...
    data.done_tasks.forEach((t) => lines.push(`    • [${t.id}] ${t.description} (${t.responsible || "Non assigné"})`));
  }
  if (data.in_progress_tasks.length > 0) {
    lines.push(`\n  🔄 EN COURS (${data.in_progress_tasks.length})`);
    data.in_progress_tasks.forEach((t) => lines.push(`    • [${t.id}] ${t.description} (${t.responsible || "Non assigné"})`));
  }
...
function generateProjectPDF(data: RapportProjet) {
  const p = data.project;
  const doc = new jsPDF({ unit: "pt", format: "a4" });
  const pageW = doc.internal.pageSize.getWidth();
  const pageH = doc.internal.pageSize.getHeight();
  ...
  if (data.done_tasks.length > 0) {
    ...
    data.done_tasks.forEach((t) => line(`- [${t.id}] ${t.description} (${t.responsible || "Non assigné"})`, 9));
  }
  if (data.in_progress_tasks.length > 0) {
    line(`En cours (${data.in_progress_tasks.length})`, 11, "bold", [40, 90, 180]);
    data.in_progress_tasks.forEach((t) => line(`- [${t.id}] ${t.description} (${t.responsible || "Non assigné"})`, 9));
  }
  if (data.overdue_tasks.length > 0) {
...
export function ReportsView({ members, projects = [], canManageOperations, isSuperadmin }: {
  ...
  const changeDateFrom = (v: string) => { setDateFrom(v); invalidate(); };
  const changeDateTo = (v: string) => { setDateTo(v); invalidate(); };
  const changeScope = (v: string) => { setScope(v); invalidate(); };
  const changeMemberId = (v: string) => { setMemberId(v); invalidate(); };
  const changeProjectId = (v: string) => { setProjectId(v); invalidate(); };

  const loadReport = async () => {
    if (scope === "member" && !memberId) { setError("Veuillez choisir un membre avant de générer le rapport."); return; }
    if (scope === "project" && !projectId) { setError("Veuillez choisir un projet avant de générer le rapport."); return; }
    if (period === "custom" && (!dateFrom || !dateTo)) { setError("Veuillez renseigner les deux dates de la plage personnalisée."); return; }
    ...
    try {
      ...
      if (scope === "project") {
        ...
        setPreview(await api.getRapportData(params));
      }
    } catch (e) { setError(api.errorMessage(e, "Erreur")); }
    finally { setLoading(false); }
  };

  const memberNameForFile = () => scope === "member" ? members.find((m) => String(m.id) === String(memberId))?.name ?? null : null;
    ...
    <div>
      ...
      <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: 24, marginBottom: 20 }}>
          ...
          <div>
            ...
            {scope === "project" && (
              <select value={projectId} onChange={(e) => changeProjectId(e.target.value)}
                ...
                {availableProjects.map((p) => <option key={p.id} value={p.id}>{p.nom}</option>)}
              </select>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          {(preview || projPreview) && (
            <>
              <button onClick={() => projPreview ? generateProjectTXT(projPreview) : preview && generateTXT(preview, memberNameForFile())} style={{
              ...
              <button onClick={() => projPreview ? generateProjectPDF(projPreview) : preview && generatePDF(preview, memberNameForFile())} style={{
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: 24 }}>
          <h3 style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 800, color: "var(--text)" }}>
            {preview.date_from && preview.date_to
              ? `Aperçu du ${formatDate(preview.date_from)} au ${formatDate(preview.date_to)}`
              : "Aperçu"}
          </h3>
              ...
              {m.difficulties.length > 0 && (
                <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 8, padding: "10px 14px" }}>
                  ...
                  {m.difficulties.map((d) => (
                    <div key={d.task_id} style={{ fontSize: 11, color: "#78350f", marginBottom: 4 }}>
                      [{d.task_id}] {d.task_description} ({d.project_name || "Sans projet"}) : {d.items.length} signalement{d.items.length > 1 ? "s" : ""}
                    </div>
                  ))}
                </div>
              )}
