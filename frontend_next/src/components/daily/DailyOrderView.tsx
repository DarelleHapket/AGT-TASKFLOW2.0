...
// Port fidèle de frontend/src/components/daily/DailyOrderView.jsx. Champs
// renommés côté OrdreJournalierSerializer : item.task_id -> item.tache ;
// item.order_index -> item.ordre ; item.start_time -> item.heure_debut ;
// item.duration_min -> item.duree_min. t.responsible (nom) -> t.responsable
// (FK id) ; myId = user directement (le compte connecté EST le membre, plus
// besoin de le rechercher par nom dans la liste des membres).
import { useState, useEffect } from "react";
import { Calendar, ChevronUp, ChevronDown, Plus, Trash2, Clock } from "lucide-react";
import * as api from "@/lib/api";
import type { OrdreJournalier, Tache, Utilisateur } from "@/lib/types";

const PRIORITY_COLOR: Record<string, string> = { critique: "#ef4444", haute: "#f59e0b", normale: "#64748b" };
const STATUS_LABEL: Record<string, string> = { todo: "À faire", in_progress: "En cours", done: "Terminée", blocked: "Bloquée" };
const STATUS_COLOR: Record<string, string> = { todo: "#64748b", in_progress: "#3b82f6", done: "#22c55e", blocked: "#ef4444" };

function addMinutes(hhmm: string | null, min: number | null | undefined): string | null {
...
export function DailyOrderView({ tasks, members, user, canViewOthers }: {
  ...
  const persist = async (newOrder: OrdreJournalier[]) => {
    const tasksList: api.DailyOrderTaskInput[] = newOrder.map((item, i) => ({
      task_id: item.tache, order_index: i, note: item.note || "",
      start_time: item.heure_debut || null, duration_min: item.duree_min || null,
    }));
  ...
  const removeItem = async (oid: number) => {
    await api.deleteDailyOrder(oid);
    setOrder((prev) => prev.filter((o) => o.id !== oid));
  };

  const updateTiming = async (idx: number, field: "heure_debut" | "duree_min", value: string | number | null) => {
  ...
  const addToOrder = async () => {
    if (!addTask) return;
    if (order.find((o) => o.tache === addTask)) { setAdding(false); return; }
    try {
      await api.setDailyOrderBulk({
        member_id: selectedMember, date: selectedDate,
        tasks: [
          ...order.map((o, i) => ({ task_id: o.tache, order_index: i, note: o.note || "", start_time: o.heure_debut || null, duration_min: o.duree_min || null })),
    ...
    } catch (e) { alert(api.errorMessage(e, "Erreur")); }
  };

  const memberTasks = tasks.filter((t) => t.responsable === Number(selectedMember) && t.statut !== "done" && !t.est_archivee);

  const selectedMemberName = members.find((m) => String(m.id) === String(selectedMember))?.name;

  const nowHHMM = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`;
  const timeState = (item: OrdreJournalier): "current" | "late" | null => {
    if (selectedDate !== today || !item.heure_debut) return null;
    const end = addMinutes(item.heure_debut, item.duree_min || 0);
    if (item.status === "done") return null;
    if (item.duree_min && end) {
      if (nowHHMM >= item.heure_debut && nowHHMM < end) return "current";
      if (nowHHMM >= end) return "late";
    } else if (nowHHMM >= item.heure_debut) {
      return "current";
    }
    return null;
  };
              ...
              <div key={item.id} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: "var(--accent)", background: "var(--accent-bg)", padding: "1px 6px", borderRadius: 4 }}>{item.tache}</span>
                  ...
                  <div style={{ display: "flex", gap: 12, fontSize: 11, color: "var(--text-3)", alignItems: "center", flexWrap: "wrap" }}>
                    <span>{item.project_name || "Sans projet"}</span>
                    <span style={{ color: STATUS_COLOR[item.status] || "#64748b", fontWeight: 600 }}>{STATUS_LABEL[item.status] || item.status}</span>
                    {canEdit ? (
                      <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        <Clock size={11} />
                        <input type="time" value={item.heure_debut || ""} onChange={(e) => updateTiming(idx, "heure_debut", e.target.value)}
                          style={{ fontSize: 11, padding: "2px 4px", borderRadius: 4, border: "1px solid var(--border)" }} />
                        <input type="number" min={0} step={15} placeholder="min" value={item.duree_min || ""} onChange={(e) => updateTiming(idx, "duree_min", e.target.value ? Number(e.target.value) : null)}
                          style={{ fontSize: 11, padding: "2px 4px", width: 56, borderRadius: 4, border: "1px solid var(--border)" }} />
                        {item.heure_debut && item.duree_min ? <span>→ {addMinutes(item.heure_debut, item.duree_min)}</span> : null}
                      </span>
                    ) : (
                      item.heure_debut && (
                        <span style={{ display: "flex", alignItems: "center", gap: 3 }}>
                          <Clock size={10} /> {item.heure_debut}{item.duree_min ? ` → ${addMinutes(item.heure_debut, item.duree_min)}` : ""}
                        </span>
                      )
                    )}
                  </div>
          ...
          {adding ? (
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <select value={addTask} onChange={(e) => setAddTask(e.target.value)}
                ...
                {memberTasks.filter((t) => !order.find((o) => o.tache === t.id)).map((t) => (
