...
class TypeMouvementFinancierViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        """BF-01 : suppression protégée (des mouvements référencent ce type)
        -> message propre au lieu d'un 500 ProtectedError non intercepté."""
        try:
            return super().destroy(request, *args, **kwargs)
        except ProtectedError:
            return Response(
                {"error": "Ce type de mouvement est utilisé par des mouvements enregistrés, impossible à supprimer."},
...
def bilan(request):
    ...
    data = calculer_bilan(date_from, date_to, niveau=niveau, projet_id=projet_id, employe_id=employe_id)
    # BF-07 : encart tableau de bord — solde courant + mouvements récents,
    # indépendamment de la période choisie pour le solde ci-dessus.
    derniers = MouvementFinancier.objects.select_related("type_mouvement", "projet", "employe__profil__utilisateur")[:5]
    data["derniers_mouvements"] = MouvementFinancierSerializer(derniers, many=True).data
