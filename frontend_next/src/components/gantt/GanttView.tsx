...
// Port fidèle de frontend/src/components/gantt/GanttView.jsx (SVG fait main).
// t.responsible (nom en texte libre) -> t.responsable (FK id) ; t.duration ->
// t.duree ; t.dependencies -> t.dependances ; t.status -> t.statut.
...
function GanttChart({ tasks, pert, memberColor }: { tasks: Tache[]; pert: PertResult; memberColor: (id: number | null) => string }) {
    ...
    <div style={{ background: "var(--bg-card)", borderRadius: 12, overflow: "auto", border: "1px solid var(--border)", boxShadow: "var(--shadow)" }}>
      <svg width={W} height={H}>
        ...
        {sorted.map((t, i) => {
          ...
          const color = memberColor(t.responsable);
          return (
            <g key={t.id}>
              ...
              <rect x={LW + es * CW + 3} y={y + 7} width={Math.max(dur * CW - 6, 4)} height={ROW - 14}
                fill={color} rx={6} opacity={t.statut === "done" ? 0.4 : 1}
...
export function GanttView({ tasks, projects, members, pert, filters, setFilters, memberColor }: {
  tasks: Tache[]; projects: Projet[]; members: Utilisateur[]; pert: PertResult;
  filters: TaskFilters; setFilters: (updater: (f: TaskFilters) => TaskFilters) => void;
  memberColor: (id: number | null) => string;
}) {
