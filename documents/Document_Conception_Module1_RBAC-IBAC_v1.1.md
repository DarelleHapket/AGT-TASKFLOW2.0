...
- **Retrait d'un rôle** = `DELETE` dans `member_roles` **+ retrait des permissions que ce rôle avait copiées** (`source='role'`), sauf si un autre rôle encore porté par l'utilisateur les accorde aussi, ou si elles ont depuis été rendues indépendantes par un retrait/ajout **direct** (`source='direct'`, IBAC — celles-là restent, conformément à BF-05). *Correction du 2026-08-10 : la première implémentation Django (`authentification.services.revoke_role`) lisait BF-05 littéralement et ne retirait jamais les permissions copiées, même orphelines de tout rôle — un utilisateur passé d'Admin à Membre gardait ainsi des permissions d'Admin. Voir `Document_Analyse_Module1_RBAC-IBAC_v1.1.md` §5.3 pour le détail et le diagramme corrigé.*
