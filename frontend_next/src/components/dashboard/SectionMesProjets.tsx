...
// Port fidèle de SectionMesProjets.jsx.
import { useState } from "react";
import { Users } from "lucide-react";
import { ProjectMembersPanel } from "@/components/projects/ProjectMembersPanel";
import type { MembreProjet, Projet, Utilisateur } from "@/lib/types";

export function SectionMesProjets({ projects, allMembers, onGetMembers, onAddMember, onUpdateMember, onRemoveMember }: {
  projects: Projet[]; allMembers: Utilisateur[];
  ...
  if (projects.length === 0) {
    return (
      <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: 18, marginBottom: 16, boxShadow: "var(--shadow)" }}>
        ...
        <div style={{ fontSize: 12, color: "var(--text-3)", textAlign: "center", padding: "16px 0" }}>Vous n&apos;avez encore aucun projet.</div>
      </div>
    );
  }
        ...
        <div key={project.id} style={{ marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", border: "1px solid var(--border)", borderRadius: 10, background: "var(--bg)" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: "var(--text)" }}>{project.nom}</div>
              <div style={{ fontSize: 11, color: "var(--text-3)" }}>{project.member_count ?? 0} membre{(project.member_count ?? 0) !== 1 ? "s" : ""}</div>
            </div>
            <button onClick={() => setOpenPanelId(openPanelId === project.id ? null : project.id)} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 700, padding: "6px 12px", borderRadius: 8, border: "1px solid var(--border)", background: openPanelId === project.id ? "var(--accent-bg)" : "transparent", color: openPanelId === project.id ? "var(--accent)" : "var(--text-2)", cursor: "pointer" }}>
