...
// a.name -> a.nom ; a.project_id -> a.projet ; a.project_name inchangé
// (déjà exposé côté serializer sous le même nom, cf. projets/serializers.py).
import { useState } from "react";
import { Plus, Pencil, Trash2, Check, X } from "lucide-react";
import type { Activite, Projet } from "@/lib/types";

interface ActivityFormData { nom: string; description: string; projet: number }

function ActivityForm({ initial, projects, onSave, onCancel }: {
  initial?: Partial<Activite>; projects: Projet[];
  onSave: (d: ActivityFormData) => void; onCancel: () => void;
}) {
  const [name, setName] = useState(initial?.nom || "");
  const [desc, setDesc] = useState(initial?.description || "");
  const [pid, setPid] = useState(initial?.projet ? String(initial.projet) : "");
  ...
  return (
    <div style={{ padding: "14px 16px", background: "var(--accent-bg)", borderBottom: "1px solid var(--border)", display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
      ...
      <div style={{ flex: 1, minWidth: 140 }}>
        ...
        <input style={inp} value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex: Backend" autoFocus />
      </div>
      ...
      <div style={{ display: "flex", gap: 8, paddingBottom: 1 }}>
        ...
        <button onClick={() => name.trim() && pid && onSave({ nom: name.trim(), description: desc.trim(), projet: Number(pid) })} disabled={!name.trim() || !pid}
          style={{ background: name.trim() && pid ? "var(--accent)" : "var(--bg)", border: "none", borderRadius: 8, padding: "7px 16px", cursor: name.trim() && pid ? "pointer" : "not-allowed", color: name.trim() && pid ? "white" : "var(--text-3)", display: "flex", alignItems: "center", gap: 5, fontWeight: 700, fontSize: 13 }}>
...
export function ActivitiesView({ activities, projects, onAdd, onUpdate, onDelete }: {
  ...
  const handleAdd = async (d: ActivityFormData) => {
    setError(null);
    try { await onAdd(d); setAdding(false); }
    ...
    <div style={{ maxWidth: 720 }}>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14, padding: "10px 14px", background: "var(--bg-card)", borderRadius: 10, border: "1px solid var(--border)" }}>
        ...
        {["all", ...projects.map((p) => String(p.id))].map((pid) => {
          const label = pid === "all" ? "Tous" : projects.find((p) => String(p.id) === pid)?.nom;
          return (
            <button key={pid} onClick={() => setFilterPid(pid)} style={{
              ...
              cursor: "pointer", fontSize: 11, fontWeight: filterPid === pid ? 700 : 400,
            }}>
              {label}
            </button>
          );
              ...
              {a.can_edit && (
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button onClick={async () => {
                    if (!window.confirm(`Supprimer l'activité "${a.nom}" ?`)) return;
                    setError(null);
