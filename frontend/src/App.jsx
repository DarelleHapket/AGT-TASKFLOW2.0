...
export default function App() {
  ...
  // B01 — Au montage, rafraîchit le profil depuis /api/auth/me.
  // Si l'admin a changé le rôle de l'utilisateur (ex. chef_projet),
  // le nouveau rôle est appliqué sans redemander le mot de passe.
  useEffect(() => {
    refreshUser();
          ...
          <div style={{ position: "relative" }}>
            ...
            {showProfile && (
              <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 12, boxShadow: "var(--shadow-md)", minWidth: 220, zIndex: 200, overflow: "hidden" }}>
                ...
                <button onClick={() => { setTab("daily"); setShowProfile(false); }} style={{ width: "100%", padding: "11px 16px", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: "var(--text)", borderBottom: "1px solid var(--border)" }}>
                  <ClipboardList size={14} /> Ma journée
                </button>
                <button onClick={() => { setTab("notes"); setShowProfile(false); }} style={{ width: "100%", padding: "11px 16px", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: "var(--text)", borderBottom: "1px solid var(--border)" }}>
                  <FileText size={14} /> Notes
                </button>
                <button onClick={logout} style={{ width: "100%", padding: "11px 16px", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: "#ef4444" }}>
